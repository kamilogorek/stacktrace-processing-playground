use std::{collections::HashMap, fs::read_to_string, io::BufRead};
...
impl<'a> Processor<'a> {
    ...
    fn preprocess_step(&mut self, frames: &[Frame]) {
    ...
    fn cache_source(&mut self, abs_path: String) {
        ...
        self.links
            .insert(abs_path.to_string(), sourcemap_ref_url.to_string());
    }
    ...
    fn process_frame(&mut self, frame: &mut Frame) {
        ...
        // let source = self.sources.get(&frame.abs_path).unwrap();
        ...
        // let minified_function_name = &frame.function;
